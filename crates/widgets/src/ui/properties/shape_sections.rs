...
pub(super) fn render_edges_section(
    ...
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if IconButton::new(icon!("edge-sharp.png"), "Sharp")
                .small()
                .selected(is_sharp)
                .show(ui)
                && !is_sharp
            ...
            if IconButton::new(icon!("edge-round.png"), "Round")
                .small()
                .selected(is_rounded)
                .show(ui)
                && !is_rounded
...
pub(super) fn render_text_section(
    ...
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ui.spacing_mut().item_spacing = Vec2::new(4.0, 0.0);
            let sizes: &[(f32, &str)] = &[(16.0, "S"), (24.0, "M"), (36.0, "L"), (48.0, "XL")];
            let current_size = if props.has_selection {
                props.font_size
            } else {
                ui_state.font_size
            };
...
pub(super) fn render_opacity_section(
    ...
    ui.vertical(|ui| {
        ui.spacing_mut().item_spacing = Vec2::new(2.0, 4.0);
        ui.label(egui::RichText::new("Opacity").size(10.0).color(label_color));
        ui.horizontal(|ui| {
...
pub(super) fn render_layer_section(ui: &mut egui::Ui, action: &mut Option<UiAction>) {
    ui.vertical(|ui| {
        ui.spacing_mut().item_spacing = Vec2::new(2.0, 4.0);
        ...
        ui.horizontal(|ui| {
            ...
            if IconButton::new(icon!("layer-front.png"), "Bring to Front")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::BringToFront);
            }
            if IconButton::new(icon!("layer-back.png"), "Send to Back")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::SendToBack);
            }
            if IconButton::new(icon!("layer-forward.png"), "Bring Forward")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::BringForward);
            }
            if IconButton::new(icon!("layer-backward.png"), "Send Backward")
                .small()
                .show(ui)
            {
...
pub(super) fn render_align_section(ui: &mut egui::Ui, action: &mut Option<UiAction>) {
    ui.vertical(|ui| {
        ui.spacing_mut().item_spacing = Vec2::new(2.0, 4.0);
        ...
        ui.horizontal(|ui| {
            ...
            if IconButton::new(icon!("align-left.png"), "Align Left")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::AlignLeft);
            }
            if IconButton::new(icon!("align-center-v.png"), "Align Center (V)")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::AlignCenterV);
            }
            if IconButton::new(icon!("align-right.png"), "Align Right")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::AlignRight);
            }
            if IconButton::new(icon!("align-top.png"), "Align Top")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::AlignTop);
            }
            if IconButton::new(icon!("align-center-h.png"), "Align Center (H)")
                .small()
                .show(ui)
            {
                *action = Some(UiAction::AlignCenterH);
            }
            if IconButton::new(icon!("align-bottom.png"), "Align Bottom")
                .small()
                .show(ui)
            {
